...
fn main() {
    ...
    let mut lines = vec![
        "# TTrades Targeted Grid (SOL 15m/4h MTF Champion)".to_string(),
        "".to_string(),
                ...
                for rr in rrs {
                    for (fee_name, fee_cfg) in fees {
                        for slip in slips {
                            ...
                            let train = execute(TTradesFractalMTF {
                                ltf_data: Arc::clone(&ltf_train),
                                htf_data: Arc::clone(&htf_train),
                                config: cfg.clone(),
                            });
                            let test = execute(TTradesFractalMTF {
                                ltf_data: Arc::clone(&ltf_test),
                                htf_data: Arc::clone(&htf_test),
                                config: cfg,
                            });
                            let tr = summarize(&train);
                            let te = summarize(&test);
                            let verdict = if te.0 > Decimal::ZERO
                                && te.1 >= Decimal::from_f32(1.2).unwrap()
                            {
                                "PROMOTE"
                            } else {
                                "REJECT"
