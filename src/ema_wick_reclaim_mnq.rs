    ...
    while i < tf.len() {
        ...
        let min_wick_ticks_eff = if cfg.use_asymmetric_wick {
            if direction == PositionDirection::Long {
                cfg.min_wick_long_ticks
            } else {
                cfg.min_wick_short_ticks
            }
        } else {
            cfg.min_wick_ticks
        };
        if wick_pen < min_wick_ticks_eff * cfg.tick_size {
            i += 1;
            continue;
        }
        ...
        let atr_stop = if atr_floor > min_stop {
            atr_floor
        } else {
            min_stop
        };
        ...
        let ob_top = if signal_open > signal_close {
            signal_open
        } else {
            signal_close
        };
        let ob_bottom = if signal_open < signal_close {
            signal_open
        } else {
            signal_close
        };
        ...
        if !matches!(cfg.entry_mode, EntryMode::Immediate) {
            ...
            while j2 <= max_j {
                ...
                let touch = if direction == PositionDirection::Long {
                    nx.low.0 <= retest_level
                } else {
                    nx.high.0 >= retest_level
                };
        ...
        while j < tf.len() {
            ...
            let sl_fill = if direction == PositionDirection::Long {
                if nx.open.0 < sl {
                    nx.open.0
                } else {
                    sl
                }
            } else if nx.open.0 > sl {
                nx.open.0
            } else {
                sl
            };
            if hit_sl && hit_tp {
                pnl_points = if direction == PositionDirection::Long {
                    sl_fill - actual_entry
                } else {
                    actual_entry - sl_fill
                };
...
fn main() {
    ...
    let base = RunCfg {
        ...
        use_asymmetric_wick: false,
        min_wick_long_ticks: Decimal::from(4),
        min_wick_short_ticks: Decimal::from(4),
    ...
    let mut rows_ema_1m: Vec<Row> = ema_sets_arc
        ...
        .map(|(ep, ema)| {
            ...
            let a = run(one_min_arc.as_slice(), ema.as_slice(), from_ts, cfg);
            evaluate(format!("ema{} rr3 wick8 atr1 cap0.10 london", ep), &a)
        })
        .collect();
    let mut rows_ema_3m: Vec<Row> = ema_sets_arc
        .par_iter()
        .map(|(ep, ema)| {
            let cfg = RunCfg {
                rr: Decimal::from(3),
            ...
            let b = run(three_min_arc.as_slice(), ema.as_slice(), from_ts, cfg);
            evaluate(format!("ema{} rr3 wick2 atr1 cap0.10 ny", ep), &b)
        })
        .collect();

    let mut sweep_cfgs: Vec<(String, RunCfg)> = Vec::new();
    for rr in rrs {
        for mw in min_wicks {
            for am in atr_mults {
                for cc in cost_caps {
                        ...
                        for sm in stop_modes {
                            for em in entry_modes {
                                ...
                                let emn = match em {
                                    ...
                                    EntryMode::ObQ25Retest => "obq25",
                                    EntryMode::ObEdgeRetest => "obedge",
                                };
                                let label = format!(
                                    "rr{} wick{} atr{} cap{} {} {} {}",
                                    rr, mw, am, cc, sname, smn, emn
                                );
                                sweep_cfgs.push((label, cfg));
                            }
                        }
                    }
                }
    ...
    let rows: Vec<(Row, Row)> = sweep_cfgs
        .par_iter()
        .map(|(label, cfg)| {
            let a = run(
                one_min_arc.as_slice(),
                ema200_5m_arc.as_slice(),
                from_ts,
                *cfg,
            );
            let b = run(
                three_min_arc.as_slice(),
                ema200_5m_arc.as_slice(),
                from_ts,
                *cfg,
            );
        ...
        .collect();
    let mut rows_1m: Vec<Row> = rows.iter().map(|(a, _)| a.clone()).collect();
    let mut rows_3m: Vec<Row> = rows.iter().map(|(_, b)| b.clone()).collect();
    ...
    for r in rows_1m.iter().take(10) {
        println!(
            ...
            r.net_usd.round_dp(2),
            r.avg_usd.round_dp(2)
        );
    }
    println!("\nTop 10 configs - 3m");
    ...
    let best_200 = best_300;

    let t_ema300 = run(
        &three_min,
        &ema_sets.iter().find(|(p, _)| *p == 300).unwrap().1,
        from_ts,
        best_300,
    );
    let t_ema200 = run(
        &three_min,
        &ema_sets.iter().find(|(p, _)| *p == 200).unwrap().1,
        from_ts,
        best_200,
    );
                    ...
                    for ow in ob_wait_t {
                        for mh in max_hold_t {
                            for s in sessions_1m {
                                ...
                                let label = format!(
                                    "1m rr{} wick{} atr{} cap{} obw{} hold{} {}",
                                    rr, wk, am, cp, ow, mh, sname
                                );
                            ...
                            for s in sessions_3m {
                                for ep in ema_period_t {
                                    ...
                                    let label = format!(
                                        "3m ema{} rr{} wick{} atr{} cap{} obw{} hold{} {}",
                                        ep, rr, wk, am, cp, ow, mh, sname
                                    );
    ...
    let base1_row = evaluate(
        "base1".to_string(),
        &run(&one_min, &ema_sets[2].1, from_ts, base_1m_new),
    );
    let base3_row = evaluate(
        "base3".to_string(),
        &run(&three_min, &ema_sets[2].1, from_ts, base_3m_new),
    );
    println!(
        "baseline 1m -> trades={}, win={}%, net=${}",
        base1_row.trades,
        base1_row.win_rate.round_dp(2),
        base1_row.net_usd.round_dp(2)
    );
    println!(
        "baseline 3m -> trades={}, win={}%, net=${}",
        base3_row.trades,
        base3_row.win_rate.round_dp(2),
        base3_row.net_usd.round_dp(2)
    );
                ...
                for tr in trend_on {
                    for ls in loss_on {
                        for mc in micro_on {
                            ...
                            c1.min_body_pct = if bp > Decimal::ZERO {
                                bp
                            } else {
                                Decimal::from(25)
                            };
                            c1.min_range_ticks = if rg > Decimal::ZERO {
                                rg
                            } else {
                                Decimal::from(5)
                            };
                            ...
                            c3.min_body_pct = if bp > Decimal::ZERO {
                                bp
                            } else {
                                Decimal::from(25)
                            };
                            c3.min_range_ticks = if rg > Decimal::ZERO {
                                rg
                            } else {
                                Decimal::from(5)
                            };
    ...
    let filt_rows: Vec<(Option<Row>, Option<Row>)> = sel_cfg
        ...
        .map(|(tag, c1, c3)| {
            ...
            let o1 = if r1.trades >= 300
                && r1.win_rate >= base1_row.win_rate
                && r1.net_usd >= base1_row.net_usd * Decimal::new(85, 2)
            {
                Some(r1)
            } else {
                None
            };
            let o3 = if r3.trades >= 250
                && r3.win_rate >= base3_row.win_rate
                && r3.net_usd >= base3_row.net_usd * Decimal::new(85, 2)
            {
                Some(r3)
            } else {
                None
    ...
    println!("\n===== Gap Sweep Winner Validation (apples-to-apples) =====");
    let ema200_3m = ema_series(&three_min, 200);

    let gap_winner_raw = RunCfg {
        rr: Decimal::from(5),
        min_wick_ticks: Decimal::from(8),
        atr_floor_mult: Decimal::new(5, 1),
        atr_period: 21,
        min_stop_ticks: Decimal::from(6),
        max_cost_r: Decimal::new(25, 2),
        session: SessionFilter::All,
        stop_mode: StopMode::Atr,
        entry_mode: EntryMode::ObMidRetest,
    ...
    let t_gap_raw = run(&three_min, &ema200_3m, from_ts, gap_winner_raw);
    print_stats("GAP WINNER RAW 3m entry + EMA200(3m)", &t_gap_raw);
    monthly_breakdown("GAP WINNER RAW 3m entry + EMA200(3m)", &t_gap_raw);

    let gap_winner_filtered = RunCfg {
        use_regime_filter: true,
        regime_min_atr_ticks: Decimal::from(2),
        regime_max_atr_ticks: Decimal::from(200),
        require_micro_confirm: true,
    ...
    let t_gap_filtered = run(&three_min, &ema200_3m, from_ts, gap_winner_filtered);
    print_stats("GAP WINNER FILTERED 3m entry + EMA200(3m)", &t_gap_filtered);
    monthly_breakdown("GAP WINNER FILTERED 3m entry + EMA200(3m)", &t_gap_filtered);

    println!("\n===== Gap Winner Selective Filter Pass =====");
    let selective_ema_dist = [
        Decimal::ZERO,
        Decimal::from(1),
        Decimal::from(2),
        Decimal::from(3),
    ];
    let selective_loss_cap = [0usize, 2usize, 3usize];
    let selective_micro = [false, true];
    let selective_sessions = [
        SessionFilter::All,
        SessionFilter::NyAm,
        SessionFilter::NyOpen,
    ];

    let mut selective_cfgs: Vec<(String, RunCfg)> = Vec::new();
    for ed in selective_ema_dist {
        for lc in selective_loss_cap {
            for mc in selective_micro {
                for s in selective_sessions {
                    let mut cfg = gap_winner_raw;
                    cfg.session = s;
                    cfg.use_ema_distance_filter = ed > Decimal::ZERO;
                    cfg.min_close_ema_dist_ticks = if ed > Decimal::ZERO {
                        ed
                    } else {
                        Decimal::from(1)
                    };
                    cfg.use_loss_streak_breaker = lc > 0;
                    cfg.max_losses_per_day = if lc > 0 { lc } else { 2 };
                    cfg.require_micro_confirm = mc;
                    let sname = match s {
                        SessionFilter::All => "all",
                        SessionFilter::NyAm => "ny",
                        SessionFilter::NyOpen => "ny_open",
                    ...
                    selective_cfgs.push((label, cfg));
                }
            }
        }
    }
    let mut selective_rows: Vec<Row> = selective_cfgs
        .par_iter()
        .map(|(label, cfg)| {
            let t = run(three_min_arc.as_slice(), &ema200_3m, from_ts, *cfg);
            evaluate(label.clone(), &t)
        })
        .collect();
    selective_rows.sort_by(|a, b| b.net_usd.partial_cmp(&a.net_usd).unwrap());
    println!("Top 10 selective around GAP WINNER RAW (3m + EMA200_3m)");
    for r in selective_rows.iter().take(10) {
        println!(
            "{} | trades={} | win={}%, net=${}, avg=${}",
            r.label,
            r.trades,
    ...
    if let Some(best_sel) = selective_rows.first() {
        println!("\nBest selective config picked: {}", best_sel.label);
    }

    println!("\n===== Feature Parity Bakeoff (Asym Wick + OB Retest Modes) =====");
    let ema200_3m_port = ema_series(&three_min, 200);
    let mut parity_cfgs: Vec<(String, RunCfg)> = Vec::new();
    let asym_pairs = [
        (Decimal::from(6), Decimal::from(6)),
        (Decimal::from(6), Decimal::from(8)),
        (Decimal::from(8), Decimal::from(6)),
    ];
    let parity_modes = [
        EntryMode::ObMidRetest,
        EntryMode::ObQ25Retest,
    ...
    let parity_slope = [false, true];
    for (wl, ws) in asym_pairs {
        for em in parity_modes {
            for slope_on in parity_slope {
                let mut cfg = RunCfg {
                    rr: Decimal::from(4),
                    min_wick_ticks: Decimal::from(6),
                    use_asymmetric_wick: true,
                    min_wick_long_ticks: wl,
                    min_wick_short_ticks: ws,
                    min_stop_ticks: Decimal::from(8),
                    atr_floor_mult: Decimal::new(4, 1),
                    atr_period: 14,
                    max_cost_r: Decimal::new(20, 2),
                    session: SessionFilter::All,
                    stop_mode: StopMode::Atr,
                    ...
                    use_ema_distance_filter: false,
                    use_candle_quality_filter: false,
                    use_trend_structure_filter: slope_on,
                ...
                if !slope_on {
                    cfg.structure_lookback = 3;
                }
                let mode = match em {
                    EntryMode::ObMidRetest => "mid",
                    EntryMode::ObQ25Retest => "q25",
                    EntryMode::ObEdgeRetest => "edge",
                    EntryMode::Immediate => "imm",
                };
                let label = format!(
                    "parity wl{} ws{} mode{} slope{}",
                    wl,
                    ws,
                    mode,
                    if slope_on { 1 } else { 0 }
                );
                parity_cfgs.push((label, cfg));
            }
        }
    }
    let mut parity_rows: Vec<Row> = parity_cfgs
        .par_iter()
        .map(|(label, cfg)| {
            let t = run(three_min_arc.as_slice(), &ema200_3m_port, from_ts, *cfg);
            evaluate(label.clone(), &t)
        })
        .collect();
    parity_rows.sort_by(|a, b| b.net_usd.partial_cmp(&a.net_usd).unwrap());
    for r in parity_rows.iter().take(10) {
        println!(
            "{} | trades={} | win={}%, net=${}, avg=${}",
            r.label,
            r.trades,
    ...
    if let Some(best) = parity_rows.first() {
        let best_net = best.net_usd;
        let current_net: Decimal = t_best_3m.iter().map(|t| t.pnl_usd).sum();
        if best_net > current_net {
            let parts: Vec<&str> = best.label.split_whitespace().collect();
            let wl = parts
                .get(1)
                .and_then(|x| x.strip_prefix("wl"))
                .and_then(|v| v.parse::<i64>().ok())
                .unwrap_or(6);
            let ws = parts
                .get(2)
                .and_then(|x| x.strip_prefix("ws"))
                .and_then(|v| v.parse::<i64>().ok())
                .unwrap_or(6);
            let mode_s = parts
                .get(3)
                .and_then(|x| x.strip_prefix("mode"))
                .unwrap_or("mid");
            let slope_on = parts
                .get(4)
                .and_then(|x| x.strip_prefix("slope"))
                .map(|v| v == "1")
                .unwrap_or(false);
            let mode = match mode_s {
                "q25" => EntryMode::ObQ25Retest,
                "edge" => EntryMode::ObEdgeRetest,
                _ => EntryMode::ObMidRetest,
            };
            let cfg = RunCfg {
                rr: Decimal::from(4),
                min_wick_ticks: Decimal::from(6),
                use_asymmetric_wick: true,
                min_wick_long_ticks: Decimal::from(wl),
                min_wick_short_ticks: Decimal::from(ws),
                min_stop_ticks: Decimal::from(8),
                atr_floor_mult: Decimal::new(4, 1),
                atr_period: 14,
                max_cost_r: Decimal::new(20, 2),
            ...
            let t = run(&three_min, &ema200_3m_port, from_ts, cfg);
            monthly_breakdown("BEST PARITY CANDIDATE", &t);
        }
    }

    println!("\n===== Ported Uncovered Winners (Main Logic) =====");
    // Port of: 3m ema200_3m wl6 ws8 h90 mid slope0 all ... cf1 ex1
    // Main runner uses a symmetric wick threshold; we test near equivalents.
    let port_a_w6 = RunCfg {
        rr: Decimal::from(4),
        min_wick_ticks: Decimal::from(6),
        min_stop_ticks: Decimal::from(8),
        atr_floor_mult: Decimal::new(4, 1),
    ...
    let port_a_w8 = RunCfg {
        min_wick_ticks: Decimal::from(8),
        ..port_a_w6
    };

    // Port of: 3m ema200_3m wl6 ws6 h90 mid slope0 all ... cf1 ex1
    let port_b = port_a_w6;

    // Closest available port of q25 setup:
    // 3m ema200_3m wl8 ws6 h90 q25 slope0.5 all ... cf1 ex1
    // q25 is not available in main runner; test closest with wick=8 + trend slope filter off.
    let port_c = RunCfg {
        min_wick_ticks: Decimal::from(8),
        ..port_a_w6
    };

    let t_port_a_w6 = run(&three_min, &ema200_3m_port, from_ts, port_a_w6);
    let t_port_a_w8 = run(&three_min, &ema200_3m_port, from_ts, port_a_w8);
    let t_port_b = run(&three_min, &ema200_3m_port, from_ts, port_b);
    let t_port_c = run(&three_min, &ema200_3m_port, from_ts, port_c);

    print_stats("PORT A (wl6/ws8 -> symmetric wick=6)", &t_port_a_w6);
    print_stats("PORT A (wl6/ws8 -> symmetric wick=8)", &t_port_a_w8);
    print_stats("PORT B (wl6/ws6 -> symmetric wick=6)", &t_port_b);
    print_stats("PORT C (q25 approx -> mid retest, wick=8)", &t_port_c);

    let net_current_best: Decimal = t_best_3m.iter().map(|t| t.pnl_usd).sum();
    let net_port_a_w6: Decimal = t_port_a_w6.iter().map(|t| t.pnl_usd).sum();
    let net_port_a_w8: Decimal = t_port_a_w8.iter().map(|t| t.pnl_usd).sum();
    let net_port_b: Decimal = t_port_b.iter().map(|t| t.pnl_usd).sum();
    let net_port_c: Decimal = t_port_c.iter().map(|t| t.pnl_usd).sum();

    if net_port_a_w6 > net_current_best {
        monthly_breakdown("PORT A (wick=6)", &t_port_a_w6);
    }
    if net_port_a_w8 > net_current_best {
        monthly_breakdown("PORT A (wick=8)", &t_port_a_w8);
    }
    if net_port_b > net_current_best {
        monthly_breakdown("PORT B (wick=6)", &t_port_b);
    }
    if net_port_c > net_current_best {
        monthly_breakdown("PORT C (wick=8)", &t_port_c);
    }
}
